...
![平台](https://img.shields.io/badge/platform-Windows%2010%2F11-blue) ![版本](https://img.shields.io/badge/version-1.1.7-green) ![许可证](https://img.shields.io/badge/license-个人使用免费%20·%20禁止转售-red) [![视频教程](https://img.shields.io/badge/Bilibili-视频教程-00A1D6?logo=bilibili&logoColor=white)](https://www.bilibili.com/video/BV1xiYm6fEuN/)
...
> 说明：视频录制于 v1.0.x（S1 时代），**操作流程一致，但界面换过一轮**：v1.1.2 起顶栏那条黑边没了（鼠标移到窗口最顶端才浮现 最小化 / 最大化 / 沉浸式 / 退出），底部的控制按钮与字幕在 v1.1.4 起收进了**左下角的小圆钮**里。看不懂视频里的界面时，对照下面「v1.1.7 新增」一节即可。
...
- **沉浸式 / 无黑边**：点顶栏的 ⛶ 让画面铺满整块屏、保持完整高度，左右两侧用画面本身的模糊环境底补满（不再是黑边），画面左侧 1/4 渐隐融进背景
...
- **沉浸式（⛶）**：窗口铺满工作区，画面**保持完整高度**，左右两侧不再是黑边，而是同一路画面的**模糊环境底**（会先提亮阴影，否则暗场景下还是一团黑），画面**左侧 1/4 逐渐透明**融进背景。再点一次 ⛶ 回到原来的窗口大小；托盘右键菜单里也有「沉浸式」。
...
- 直接下载：[`release/云端数字人-分享版-v1.1.7.zip`](https://github.com/beiyege-01/clouddh-share/raw/main/release/%E4%BA%91%E7%AB%AF%E6%95%B0%E5%AD%97%E4%BA%BA-%E5%88%86%E4%BA%AB%E7%89%88-v1.1.7.zip)（约 20 MB）
- 或到 [Releases 页面](https://github.com/beiyege-01/clouddh-share/releases) 下载同一份（`CloudDH-Share-v1.1.7.zip`）

> 校验：zip 的 SHA256 见 Releases 说明；解压后 `云端数字人-分享版.exe` 的文件版本号是 **1.1.7.0**。
...
5. 想玩 S2 的新花样：点**左下角的小圆钮**展开控制台，里面 **🎭** 是实时换装/换背景，**📝** 是改人设；⚙ 面板里还有待机小动作、冷场主动搭话、长期记忆、录制回放。
...
| **底部那排按钮和字幕不见了** | v1.1.4 起默认收进**左下角的小圆钮**：点一下展开（有字幕输入时会亮一个小橙点），点画面任意处自动收起 |
| **窗口拖不动大小** | 拖**底边**或**左下 / 右下 / 左上角**（光标会变成对应箭头）。左右两条边是故意不做把手的——比例锁定下拖侧边会被立刻弹回去。窗口比例始终等于形象比例，会记在配置里 |
| **进沉浸式后左右不是黑边而是一层模糊底** | 这是设计：画面保持完整高度、宽度不够铺满屏幕，两侧用同一路画面的模糊放大版补满（比黑边更像壁纸）。想回到普通窗口再点一次 ⛶ |
